...
type scope struct {
	hashTable     helper.HashTable // hashTable store all defined variables in the current scope
...
func (st *SymbolTable) Add(s *Symbol) {
	st.head.hashTable.Add(s.name, s)
}
...
func (st *SymbolTable) Lookup(name string) (entry *Symbol, ok bool) {
	...
	for {
		result, ok := currentScope.hashTable.Get(name)
		if !ok {
